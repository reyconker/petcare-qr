...
import {
  ...
  Syringe, Calendar, ChevronRight, Info
} from 'lucide-react';
...
export default async function HoyPage() {
  ...
  const alerts = [
    ...
    ...upcomingVaccines.map((v) => ({
      ...
      text: `${v.type === 'vacuna' ? 'Vacuna' : 'Desparasitación'} "${v.name}" próxima el ${v.nextDate}.`,
      href: '/vacunas',
    })),
    ...(nextControl
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-500 mt-0.5 text-sm">
          Control diario de <strong>{data.dog.name}</strong> — {new Date().toLocaleDateString('es-CL', { weekday: 'long', day: 'numeric', month: 'long' })}
        </p>
      </div>
      ...
      {alerts.length > 0 && (
        <div className="space-y-2">
          {alerts.map((alert) => {
            const bgColor = alert.level === 'error'
      ...
      <section>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
            <Pill className="w-5 h-5 text-teal-600" /> Tratamientos de hoy
          </h2>
        ...
        {activeTreatments.length === 0 ? (
          <div className="bg-white border border-dashed border-gray-200 rounded-xl p-6 text-center">
            ...
            <p className="text-gray-500 text-sm">No hay tratamientos activos.</p>
            <Link href="/tratamientos" className="text-teal-600 text-sm font-medium hover:underline mt-1 inline-block">
              Agregar tratamiento →
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {activeTreatments.map((treatment) => {
              ...
              return (
                <div
                  key={treatment.id}
                  className={`bg-white border rounded-xl p-4 shadow-sm transition-shadow hover:shadow-md ${isPending && !outOfStock ? 'border-teal-200' : 'border-gray-100'}`}
                >
                  ...
                  <p className="text-gray-400 text-xs mb-3">
                    Cada {treatment.frequencyHours}h · {dosesGivenToday} de {Math.ceil(dosesPerDay)} dosis hoy
                  </p>
                  ...
                  {isPending && !outOfStock ? (
                    ...
                    >
                      ...
                      >
                        <CheckCircle className="w-4 h-4" /> Marcar dosis dada
                      </button>
                    </form>
                  ) : (
                    <div
                    ...
                    >
                      {outOfStock ? 'Stock insuficiente' : '✓ Todas las dosis completadas'}
                    </div>
                  )}
                </div>
              );
      ...
      {/* Resumen de alimento + vacunas */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {/* Alimento */}
        <Link
          href="/alimento"
          className={`bg-white border rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow flex items-start gap-3 ${isFoodLow ? 'border-orange-200' : 'border-gray-100'}`}
        >
          <div className={`p-2.5 rounded-xl flex-shrink-0 ${isFoodLow ? 'bg-orange-100 text-orange-600' : 'bg-amber-100 text-amber-600'}`}>
          ...
          <div className="min-w-0 flex-1">
            ...
            {hasFood ? (
              <>
                ...
                <p className="text-xs text-gray-400">Ración: {data.food.dailyRationGrams}g/día · {data.food.timesPerDay} comida{data.food.timesPerDay !== 1 ? 's' : ''}</p>
              </>
            ) : (
              <p className="text-xs text-gray-400 mt-0.5">Sin alimento registrado</p>
            )}
          </div>
        ...
        <Link
          href="/vacunas"
          className={`bg-white border rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow flex items-start gap-3 ${urgentVaccines.length > 0 ? 'border-red-200' : upcomingVaccines.length > 0 ? 'border-amber-200' : 'border-gray-100'}`}
        >
          <div className={`p-2.5 rounded-xl flex-shrink-0 ${urgentVaccines.length > 0 ? 'bg-red-100 text-red-600' : upcomingVaccines.length > 0 ? 'bg-amber-100 text-amber-600' : 'bg-blue-100 text-blue-600'}`}>
            <Syringe className="w-5 h-5" />
          </div>
          <div className="min-w-0 flex-1">
            <p className="font-semibold text-gray-800 text-sm">Vacunas</p>
            {urgentVaccines.length > 0 ? (
              <p className="text-red-600 font-bold text-sm">{urgentVaccines.length} vencida{urgentVaccines.length !== 1 ? 's' : ''}</p>
            ) : upcomingVaccines.length > 0 ? (
              <p className="text-amber-600 font-bold text-sm">{upcomingVaccines.length} próxima{upcomingVaccines.length !== 1 ? 's' : ''}</p>
            ) : (
              <p className="text-green-600 font-bold text-sm">Al día</p>
            )}
            <p className="text-xs text-gray-400">{data.vaccines.length} registrada{data.vaccines.length !== 1 ? 's' : ''}</p>
          </div>
          <ChevronRight className="w-4 h-4 text-gray-400 flex-shrink-0 mt-1" />
        </Link>
      </div>
